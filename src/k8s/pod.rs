...
pub async fn instantiate_pod(client: &Client, pod_spec: &Pod) -> Result<Pod> {
	let pod_api: Api<Pod> = Api::namespaced(client.client(), client.namespace());
...
pub async fn runrunrun(client: &Client, pod_name: String) -> Result<AttachedProcess> {
	let pod_api: Api<Pod> = Api::namespaced(client.client(), client.namespace());
